...
StyledRect {
    ...
    readonly property int activeWinCount: Hypr.workspaces.values.find(w => w.id === activeWsId)?.lastIpcObject.windows ?? 0

    clip: true
    x: offset + 6
    implicitHeight: Config.bar.sizes.innerWidth - Appearance.padding.small * 2
    implicitWidth: activeWinCount === 0 ? implicitHeight : size
