    ...
    Loader {
        ...
        sourceComponent: Item {
            StyledClippingRect {
                ...
                readonly property int specialWinCount: view.currentItem?.modelData?.lastIpcObject.windows ?? 0

                anchors.top: parent.top
                anchors.bottom: parent.bottom

                x: (view.currentItem?.x ?? 0) - view.contentX + 11
                implicitWidth: specialWinCount === 0 ? height : (view.currentItem?.size ?? 0)
